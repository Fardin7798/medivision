...
| 11 | `AttributeError: st.session_state has no attribute "affine"` | `st.session_state` was initialized without `affine` key; accessed directly on TotalSegmentator call. | Added `affine=np.eye(4)` to default session state and used `getattr(st.session_state, "affine", np.eye(4))` getter in `streamlit_app.py` and `app/main.py`. |
